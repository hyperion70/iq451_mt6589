...
MBOOL TpipeDrvImp::getNr3dTop(
    TPIPE_DRV_CQ_ENUM cq,
    MUINT32 *pEn
)
{
    Mutex::Autolock lock(mLock);
...
)
{
    ...
    LOG_INF("[Top]bnr(%d),lsc(%d),lsci(%d),lce(%d),lcei(%d),nbc(%d),seee(%d),cfa(%d)",
    ...
    //
    LOG_INF("per_tpipe(%d),en1(0x%08x),en2(0x%08x),dma(0x%08x)",
            tpipeDescriptor.config_no_per_tpipe, en1, en2, dma);
