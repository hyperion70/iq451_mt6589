...
MBOOL
LscMgr::
fillTblInfoByLscScenarionCT(SHADING_TBL_SPEC &tbl_sepc, ELscScenario_T lsc_scenario, MUINT8 ct)
{
    SENSOR_CROP_INFO crop_info;
    UINT32* tbl_addr;
    UINT8   LSCParamIdx = 0;
    MUINT32 sensor_scenario;
    MY_LOG("[%s]  ", __FUNCTION__);

    sensor_scenario = getSensorScenarioByLscScenario(lsc_scenario);

    tbl_sepc.img_width   = m_SensorCrop[sensor_scenario].u4SrcW;
    tbl_sepc.img_height  = m_SensorCrop[sensor_scenario].u4SrcH;
    tbl_sepc.offset_x    = m_SensorCrop[sensor_scenario].u4GrabX;
    tbl_sepc.offset_y    = m_SensorCrop[sensor_scenario].u4GrabY;
    ...
    tbl_sepc.grid_x      = (int)m_rIspLscCfg[lsc_scenario].ctl2.bits.SDBLK_XNUM+1;
    tbl_sepc.grid_y      = (int)m_rIspLscCfg[lsc_scenario].ctl3.bits.SDBLK_YNUM+1;
    tbl_sepc.lwidth      = m_rIspLscCfg[lsc_scenario].lblock.bits.SDBLK_lWIDTH;
    tbl_sepc.lheight     = m_rIspLscCfg[lsc_scenario].lblock.bits.SDBLK_lHEIGHT;
    tbl_sepc.ratio_idx   = 1;//m_rIspLscCfg[lsc_scenario].ratio.bits.RATIO00/32;
    tbl_sepc.grgb_same   = SHADING_GRGB_SAME_YES;
#if DEBUG_ALIGN_FUNC
    tbl_sepc.table       = (MUINT32*)golden_cct;
#else
    tbl_sepc.table       = (MUINT32*)((MUINT32)getLut(lsc_scenario) + (MUINT32)ct * getPerLutSize(lsc_scenario));
#endif
...
importEEPromData(void)
{
    ...
    else if (table_type & (1<<1))   // mtk lsc
    {
        ...
        eeprom.ctl2.bits.SDBLK_XNUM     = ((mtk_lsc->CapIspReg[1] >> 28) & 0x0000000F) + 1;
        eeprom.ctl3.bits.SDBLK_YNUM     = ((mtk_lsc->CapIspReg[1] >> 12) & 0x0000000F) + 1;
        ...
        golden.ctl2.bits.SDBLK_XNUM         = ((m_rIspShadingLut.SensorGoldenCalTable.IspLSCReg[1] >> 28) & 0x0000000F) + 1;
        golden.ctl3.bits.SDBLK_YNUM         = ((m_rIspShadingLut.SensorGoldenCalTable.IspLSCReg[1] >> 12) & 0x0000000F) + 1;
        ...
        {
            MY_LOG("[%s:golden] No golden setting, using eeprom setting instead", __FUNCTION__);
        ...
        align_conf.cali.img_width   = m_SensorCrop[cali_scenario].u4SrcW;
        align_conf.cali.img_height  = m_SensorCrop[cali_scenario].u4SrcH;
        align_conf.cali.offset_x    = m_SensorCrop[cali_scenario].u4GrabX;
        align_conf.cali.offset_y    = m_SensorCrop[cali_scenario].u4GrabY;
        ...
        align_conf.cali.grid_x      = (int)eeprom.ctl2.bits.SDBLK_XNUM+1;
        align_conf.cali.grid_y      = (int)eeprom.ctl3.bits.SDBLK_YNUM+1;
        align_conf.cali.lwidth      = eeprom.lblock.bits.SDBLK_lWIDTH;
        align_conf.cali.lheight     = eeprom.lblock.bits.SDBLK_lHEIGHT;
        align_conf.cali.ratio_idx   = 1;
        align_conf.cali.grgb_same   = SHADING_GRGB_SAME_YES;
        align_conf.cali.table       = (UINT32*)mtk_lsc->CapTable;
        align_conf.cali.data_type   = SHADING_TYPE_GAIN;

        align_conf.golden.img_width   = m_SensorCrop[cali_scenario].u4SrcW;
        align_conf.golden.img_height  = m_SensorCrop[cali_scenario].u4SrcH;
        align_conf.golden.offset_x    = m_SensorCrop[cali_scenario].u4GrabX;
        align_conf.golden.offset_y    = m_SensorCrop[cali_scenario].u4GrabY;
        ...
        align_conf.golden.grid_x      = (int)golden.ctl2.bits.SDBLK_XNUM+1;
        align_conf.golden.grid_y      = (int)golden.ctl3.bits.SDBLK_YNUM+1;
        align_conf.golden.lwidth      = golden.lblock.bits.SDBLK_lWIDTH;
        align_conf.golden.lheight     = golden.lblock.bits.SDBLK_lHEIGHT;
        align_conf.golden.ratio_idx   = 1;
        align_conf.golden.grgb_same   = SHADING_GRGB_SAME_YES;
        ...
        for (ct_idx = 0; ct_idx < SHADING_SUPPORT_CT_NUM && ret == S_LSC_CONVERT_OK; ct_idx++)
        {
            ...
            fillTblInfoByLscScenarionCT(align_conf.output, (ELscScenario_T)lsc_scenario, ct_idx);
            //memcpy(&align_conf.output, &align_conf.output, sizeof(align_conf.input));
        ...
        for (lsc_scenario = 0; lsc_scenario < LSC_SCENARIO_NUM && ret == S_LSC_CONVERT_OK; lsc_scenario++)
        {
            if (lsc_scenario != LSC_SCENARIO_04) {
                for (ct_idx = 0; ct_idx < SHADING_SUPPORT_CT_NUM && ret == S_LSC_CONVERT_OK; ct_idx++)
                {
                    fillTblInfoByLscScenarionCT(trfm.input, LSC_SCENARIO_04, ct_idx);
                    fillTblInfoByLscScenarionCT(trfm.output, (ELscScenario_T)lsc_scenario, ct_idx);
...
setIspProfile(EIspProfile_T const eIspProfile)
{
    ...
    ////// work around
    if (m_bMetaMode == MTRUE && eIspProfile == EIspProfile_NormalCapture) {
        switch (m_SensorMode) {
...
setMetaIspProfile(EIspProfile_T const eIspProfile, MUINT32 sensor_mode)
{
    ...
    if (MFALSE == setIspProfile(eIspProfile))
        return MFALSE;

    if (eIspProfile == EIspProfile_NormalCapture) {
        MY_LOG("[LscMgr:%s], EIspProfile_NormalCapture, sensor_mode %d", __FUNCTION__,
            sensor_mode);
        switch (sensor_mode) {
            case ESensorMode_Preview:
                m_eSensorOp = ACDK_SCENARIO_ID_CAMERA_PREVIEW;
        ...
        MY_LOG("[LscMgr:%s] m_eLscScenario %d, m_eSensorOp %d", __FUNCTION__, m_eLscScenario, m_eSensorOp);
    }
    return MTRUE;
}
...
enableLsc(MBOOL const fgEnable)
{
    ...
    if (fgRet) {
        MY_LOG("[LscMgr:%s]  "
                "Shading param 0x%x, 0x%x, 0x%x, 0x%x,0x%x ,0x%x ,0x%x ,0x%x, 0x%x \n",
                __FUNCTION__,
