...
MRESULT AeMgr::setFDInfo(MVOID* a_sFaces)
{
    ...
    if((pFaces->number_of_faces == 0) && ((m_eAEFDArea.i4Left != 0x00) || (m_eAEFDArea.i4Right != 0x00) || (m_eAEFDArea.i4Top != 0x00) || (m_eAEFDArea.i4Bottom != 0x00))){
        memset(&m_eAEFDArea, 0, sizeof(AEMeterArea_T));
        MY_LOG("Face number:%d Win:%d %d %d %d \n", pFaces->number_of_faces, pFaces->faces->rect[0], pFaces->faces->rect[1], pFaces->faces->rect[2], pFaces->faces->rect[3]);
        if(m_pIAeAlgo != NULL) {
    ...
    } else if((m_eAEFDArea.i4Left != pFaces->faces->rect[0]) || (m_eAEFDArea.i4Right != pFaces->faces->rect[2]) ||
