...
static int charger_low_counter=0;
static int charger_high_counter=0;

/*****************************************************************************
 *  BATTERY VOLTAGE
 ****************************************************************************/
#if defined(SIMCOM_FOR1T)
#define BATTERY_LOWVOL_THRESOLD             3550
#else
...
#define CHARGER_UNDER_VOL                   2
#define BATTERY_OVER_TEMP                   3
#define BATTERY_UNDER_TEMP                  4 
...
int BAT_CheckBatteryStatus(void)
{
    ...
    {
        //xlog_printk(ANDROID_LOG_WARN, "Power/Battery", "[BATTERY] NTC fail !!\n\r");           
        BMT_status.bat_charging_state = CHR_ERROR;
        return PMU_STATUS_FAIL;       
    }
    else if (BMT_status.temperature <= MIN_CHARGE_TEMPERATURE) 
    {
        //xlog_printk(ANDROID_LOG_WARN, "Power/Battery", "[BATTERY] Battery Under Temperature !!\n\r");           
        BMT_status.bat_temperature_status=BATTERY_UNDER_TEMP;  
        BMT_status.bat_charging_state = CHR_ERROR;
    ...
    {
        if (BMT_status.bat_temperature_status==BATTERY_UNDER_TEMP) 
        {                
            BMT_status.bat_charging_state = CHR_PRE;
            BMT_status.bat_temperature_status=0;
        }
    }
    #endif                
    if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)
    {
		...
		BMT_status.bat_temperature_status = BATTERY_OVER_TEMP;
        BMT_status.bat_charging_state = CHR_ERROR;
        return PMU_STATUS_FAIL;       
    }
	else
	{
	    if (BMT_status.bat_temperature_status == BATTERY_OVER_TEMP) 
		{                
			BMT_status.bat_charging_state = CHR_PRE;
			BMT_status.bat_temperature_status=0;
		}
    ...
    if (upmu_is_chr_det() == KAL_TRUE)
    {
			...
			{
				charger_low_counter = 0;
			}
			
			if(charger_low_counter > 4)
			{
			...
			else if(BMT_status.charger_vol > V_CHARGER_MIN )
			{
				if (BMT_status.charger_protect_status == CHARGER_UNDER_VOL) 
				{                  
...
void BAT_thread(void)
{
    ...
    else
    {
        ...
        {
#if 1//defined(DISABLE_POST_CHARGE)
