...
#if defined(SIMCOM_POWER_ON_VIBR_SUPPORT)
void pl_open_vibr(void)
{
	int ret_val=0;	
	ret_val=pmic_config_interface(DIGLDO_CON40, PMIC_RG_VIBR_VOL_3000 , PMIC_RG_VIBR_VOSEL_MASK, PMIC_RG_VIBR_VOSEL_SHIFT);
	ret_val=pmic_config_interface(DIGLDO_CON39, 0x01, PMIC_RG_VIBR_EN_MASK, PMIC_RG_VIBR_EN_SHIFT);
	printf("[pmic6329_init] pl_open_vibr\n");
}


void pl_close_vibr(void)
{
	int ret_val=0;	
	ret_val=pmic_config_interface(DIGLDO_CON39, 0x00, PMIC_RG_VIBR_EN_MASK, PMIC_RG_VIBR_EN_SHIFT);
	printf("[pmic6329_init] pl_close_vibr\n");
}
#endif
