    ...
    AudioSystem::stream_type stream)
{
    ...
    //ALOGD("getStrategy() stream type=%d",stream);
    switch (stream) {
        ...
        case STRATEGY_PHONE:
            ...
            switch (mForceUse[AudioSystem::FOR_COMMUNICATION]) {
                ...
                default:    // FORCE_NONE
                    ...
                    if (device) break;
            if (mPhoneState != AudioSystem::MODE_IN_CALL && mPhoneState != AudioSystem::MODE_IN_CALL_2) {
                                                                           ...
                                                                           uint32_t channelMask)
{
    ...
    ALOGD("getInputProfile mHwModules size = %d", mHwModules.size());
    for (size_t i = 0; i < mHwModules.size(); i++) {
        if (mHwModules[i]->mHandle == 0) {
            continue;
        }
        for (size_t j = 0; j < mHwModules[i]->mInputProfiles.size(); j++) {
            ALOGD("getInputProfile mInputProfiles size = %d",  mHwModules[i]->mInputProfiles.size());
            IOProfile *profile = mHwModules[i]->mInputProfiles[j];
            ALOGD("isCompatibleProfile i= %d", i);
            if (profile->isCompatibleProfile(device, samplingRate, format,
...
{
    ALOGD("mSupportedDevices = 0x%x device = 0x%x", mSupportedDevices, device);
    if ((mSupportedDevices & device) != device) {
        return false;
    }
    if ((mFlags & flags) != flags) {
        ALOGD("mFlags = 0x%x flags = 0x%x", mFlags, flags);
        return false;
    }
    if (samplingRate != 0) {
        size_t i;
        for (i = 0; i < mSamplingRates.size(); i++) {
            ALOGD("mSamplingRates[i] = 0x%x samplingRate = 0x%x", mSamplingRates[i], samplingRate);
    ...
    if (format != 0) {
        ...
        for (i = 0; i < mFormats.size(); i++) {
            ALOGD("mFormats[i] = 0x%x format = 0x%x", mFormats[i], format);
    ...
    if (channelMask != 0) {
        ...
        for (i = 0; i < mChannelMasks.size(); i++) {
            ALOGD("mChannelMasks[i] = 0x%x channelMask = 0x%x", mChannelMasks[i], channelMask);
...
void AudioPolicyManagerBase::defaultAudioPolicyConfig(void)
{
    ...
    inputprofile->mSupportedDevices = (AUDIO_DEVICE_IN_COMMUNICATION | AUDIO_DEVICE_IN_AMBIENT |
                                                        ...
                                                        AUDIO_DEVICE_IN_AUX_DIGITAL | AUDIO_DEVICE_IN_VOICE_CALL | AUDIO_DEVICE_IN_BACK_MIC);
