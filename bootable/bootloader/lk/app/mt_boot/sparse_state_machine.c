...
static unsparse_data_t m_unsparse_data={0};
	...
	case UNSPARSE_WAIT_SPARSE_HEADER:
		{
			if (size >=sizeof (sparse_header_t))
			{
				...
				memset(&m_unsparse_data, 0x00, sizeof(unsparse_data_t));
				memcpy(&m_unsparse_data.sparse_hdr, buf, sizeof (sparse_header_t));
